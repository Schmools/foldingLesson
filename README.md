...
mountain fold perpendicular to the edge of paper and then by taking the
back layer of that same paper and making a simple valley fold along a line
perpendicular to the direction we want the strip to point in and through
one of the two points where our 1st mountain fold intersected the edge of
...
mountain fold perpendicular to the edge of the paper and then folding the
bottom layer along a valley fold 45 degrees to the edge of the paper and
through one of the points of intersection of original mountain fold and the
edge, and then making a similar fold to now the 2 back layers of paper
...
to be a different color we use our color-reversal gadget. More formally our
...
vertices of our polygon (such that they don't cross other placed lines)
...
want to be efficient with paper usage, this becomes a traveling salesman
problem). Starting at the vertex of a triangle opposite the edge shared
with the triangle we want to visit next, we zig zag our strip across the
...
a turn gadgets to become aligned parallel to the shared edge with the next
triangle we want to visit, and repeat until we have finished covering the
surface of the polygon). If we ever want a triangle to be a different
color we simply use the color-change gadget as we transition on to that
triangle. Finally we can notice that though we have covered the entire
surface area of the polygon our strips actually overhang the edges where
we have done the turn gadgets we can hide the excess paper underneath each
triangle by doing successive mountain folds along the edges of the
triangles until we only have the desired shape remaining. 
...
Surpisingly this algorithm can be quite efficient, as we decrease the
width of our strip the amount overhanging (the wasted paper) decreases (as
it is only the width of our strip of paper), so as our paper gets
arbitrarily thin our wasted paper usage gets arbitrarily low as long as we
only visit each rectangle once. There are several cool methods to show
that we only have to visit each triangle once, but one that is really easy
to understand, is if we consider our strip of paper to be infinitely thin
we can actually pick any random order of triangles to visit and go
straight to each one without regards to an efficient path. Since in this
case our paper is infinitely thin we actually waste no paper doing this. 
...
though is theoretically possible by the strip method by using very wide
strips, really hurts it's efficiency) and such that the entire folding is
...
through it). This algorithm is quite difficult to prove but not very
hard to understand. 
...
we can develop a "tuck" gadget that works for bringing all of the vertices
...
we have to be careful of here is that the "tucked" flaps don't intersect
...
initial faces of the polyhedron to be efficient with paper usage even if
it is theoretically possible (in a bit we will talk a little more about
...
counter intuitively after all paper is 2D, but gaining an idea of 1D
...
distance to the next folds are further then the distance between the two
...
Since both *crimps* and *end folds* don't cause any risk of intersection,
our piece of paper after we perform these folds is still flat foldable.
Therefore if we want to check if a pattern is flat foldable we simply
continuously *crimp* and *end fold* our paper, until the paper is entirely
...
every *crimp* and *end fold* is flat foldable it doesn't matter what
order we do this in, if we have an issue with our folding it won't come
from either *crimps* or *end folds* but some other self intersecting, or
otherwise bad fold. 
...
furthermore if we have a 2D sheet where we only have creases perpendicular
...
with creases emanating from the center. The idea is that this allows us to
understand foldability around a vertex in a larger crease pattern. How we
consider folding this strip is very cool, if we consider the circumference
of the disk as a wrapped line segment and where the creases intersect the
circumference as points, we can all of a sudden just see our single
vertex problem as just a 1D folding problem, we just need to pick
a location to "split" our circumference. The other stipulation is that we
need to be able to rejoin our circumference at the end, so to be flat
...
the diagrams below). For this reason we choose to cut the circumference at
...
paper at that crease this rejoining doesn't intersect other folds. 

To this end we have two theorems that are required for a single vertex
fold to be *flat foldable.* To understand them we have to realize that the
length of line segments between creases in our 1D folding correspond to
the central angles of our vertex fold:
...
- Kawasaki's theorem says that the sum of all of the odd angles, must
  ...
  diagram below but the deal is that angles correspond to distances
  traveled between creases, in our 1D folding, and creases are where we
  switch direction. Therefore to end up where we started the odd and even
  creases must equal each other, the left and right "switchbacks" must
  cancel each other out. Also since all of the central angles mush sum to
...
all 120 degrees apart, this won't fold flat, but notice what you have to
...
to look at how whole 2D crease patterns by checking, that all vertices
...
flat), and since in crimps we must have both *mountain* and *valley*
folds we can begin to figure out creases must not be equal (must be of
opposite assignment), the other bit of knowledge we have is that creases
between vertices must be the same fold type (a crease cant transitions
from *moutnain* to *valley* between vertices) and so armed with this
...
A simple example of a fold that is clearly not possible by this method
is shown below. The other tricky thing here is sometimes we have multiple
possible crimp patterns and sometimes performing these crimps joins new
...
as many vertex "loops" together and "chase folds" until we either run
into an issue or the whole thing is locally flat foldable.
...
a problem called "all-positive not-all-equal 3-satisfiability," a mouthful
for sure, but not too hard to understand it basically says given some
triples is there a boolean assignment to the variables in the triples so
...
smallest possible square of paper into a "uniaxial" origami base whose
...
First we define some more parts of a uniaxial base, a uniaxial base is
...
We also need to introduce a lemma, the distance between any two points on
...
4. Finally we turn all of our triangles into "rabbit-ear molecules" (or
   ...
   perpendiculars to the incenter (the step we performed in the demo) and
   make them valley folds. Also in an extra step here at each hinge point
   we draw in perpendiculars to the angular bisectors, and make them
   valley folds. 
...
(https://langorigami.com/artwork/knl-dragon-opus-132/) being folded (these
tend to be designed with tree maker to get flaps for all of the appendages
and then folded from there).
